...
public class forecastTreeC45 {
    ...
    public DecisionTreeNode evaluation(String[] title ,Object[] data,Object[] printData,int dLocal){
        ...
        if (root!=null && root.getChildNodesArray()!=null){
    ...
    public void forecast(Object[] printData, DecisionTreeNode node) {
        ...
        for (int i = 0; i < childs.length; i++) {
            if (childs[i] != null && printData[index]!=null) {
                if (String.valueOf(printData[index]).indexOf(childs[i].parentArrtibute) !=-1) {
    ...
    public String[] getArrtibutesArray(int index) {
        ...
        for (int i = 0; i < trainingArray.length; i++) {
            ...
            if (strs!=null && index>-1 &&  index<strs.length){
                set.add(strs[index]);
            }
        }
